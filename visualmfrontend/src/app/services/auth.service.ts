...
export class AuthService {
  ...
  constructor(private http: HttpClient) {
    // Remove login: populate a default anonymous user to avoid null checks in templates.
    this.currentUser = new User();
    this.currentUser.setId(0);
    this.currentUser.setEmail('guest@local');
  ...
  public isLoggedIn(): boolean {
    // Login removed: treat user as logged in for routing/UI toggles.
    return true;
  }

  public isAdmin(): boolean {
    // For now, preserve an unauthenticated admin check by user data if available.
